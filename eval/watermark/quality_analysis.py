...
from modules.watermark.base import BaseWatermark
from data.data_registry.base_loader import BaseDataset
from utils.watermark.tools.text_editor import TextEditor
from utils.watermark.exceptions.exceptions import InvalidTextSourceModeError, InvalidDirectAnalyzerTypeError, InvalidReferencedAnalyzerTypeError
from eval.watermark_text_quality_analyzer import (TextQualityAnalyzer, DirectTextQualityAnalyzer, ReferencedTextQualityAnalyzer, 
...
class TextQualityAnalysisPipeline:
    ...
    def evaluate(self, watermark: BaseWatermark):
        ...
        if self.return_type == QualityPipelineReturnType.FULL:
            ...
            if evaluation_result and isinstance(evaluation_result[0].watermarked_quality_score, dict):
                # initial 
                avg_scores = {
                    'watermarked': {},
                    'unwatermarked': {}
                ...
                for result in evaluation_result:
                    all_metrics.update(result.watermarked_quality_score.keys())
                    all_metrics.update(result.unwatermarked_quality_score.keys())
                
                # compute the mean of metrics
                for metric in all_metrics:
                    avg_scores['watermarked'][metric] = sum(
                        r.watermarked_quality_score.get(metric, 0) for r in evaluation_result
                    ) / len(evaluation_result)
                    
                    avg_scores['unwatermarked'][metric] = sum(
                        r.unwatermarked_quality_score.get(metric, 0) for r in evaluation_result
                    ) / len(evaluation_result)
                
                return avg_scores
            else:
                # keep BLUE process and filter Mavue None value
                valid_watermarked_scores = [res.watermarked_quality_score for res in evaluation_result if res.watermarked_quality_score is not None]
                avg_watermarked = sum(valid_watermarked_scores) / len(valid_watermarked_scores) if valid_watermarked_scores else 0.0
                valid_unwatermarked_scores = [res.unwatermarked_quality_score for res in evaluation_result if res.unwatermarked_quality_score is not None]
                avg_unwatermarked = sum(valid_unwatermarked_scores) / len(valid_unwatermarked_scores) if valid_unwatermarked_scores else 0.0
                
                # Check if the results are float and handle accordingly
                if isinstance(avg_watermarked, float):
                    avg_watermarked = avg_watermarked
                else:
                    avg_watermarked = avg_watermarked.item() if hasattr(avg_watermarked, 'item') else avg_watermarked
                    
                return {
                    'watermarked': avg_watermarked,
                    'unwatermarked': avg_unwatermarked
                }
