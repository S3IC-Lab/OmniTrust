...
from utils.watermark.exceptions.exceptions import TypeMismatchException, ConfigurationError
import numpy as np
...
class BaseSuccessRateCalculator:
    ...
    def __init__(self, labels: List[str] = ['TPR', 'TNR', 'FPR', 'FNR', 'P', 'R', 'F1', 'ACC', 'AUROC', 'Sn', 'Sp', 'MCC']) -> None:
        """
...
class FundamentalSuccessRateCalculator(BaseSuccessRateCalculator):
    ...
    def __init__(self, labels: List[str] = ['TPR', 'TNR', 'FPR', 'FNR', 'P', 'R', 'F1', 'ACC', 'Sn', 'Sp', 'MCC']) -> None:
        """
    ...
    def _compute_metrics(self, inputs: List[DetectionResult]) -> Dict[str, float]:
        ...
        Sn = TP / (TP + FN) if TP + FN else 0.0  # Sensitivity
        Sp = TN / (TN + FP) if TN + FP else 0.0  # Specificity
        MCC = (TP * TN - FP * FN) / np.sqrt((TP + FP) * (TP + FN) * (TN + FP) * (TN + FN)) if (TP + FP) * (TP + FN) * (TN + FP) * (TN + FN) else 0.0

        #  AUROC (only for floats)
        if all(isinstance(d.detect_result, float) for d in inputs):
            y_true = [int(d.gold_label) for d in inputs]
            y_score = [d.detect_result for d in inputs]
            AUROC = roc_auc_score(y_true, y_score)
        else:
            AUROC = 0.0  # for bool seeting to 0

        return {
            'TPR': TPR, 'TNR': TNR, 'FPR': FPR, 'FNR': FNR,
            'P': P, 'R': R, 'F1': F1, 'ACC': ACC, 'AUROC': AUROC, 'Sn': Sn, 'Sp': Sp, 'MCC': MCC
        }
...
class DynamicThresholdSuccessRateCalculator(BaseSuccessRateCalculator):
    ...
    def __init__(self, 
                 labels: List[str] = ['TPR', 'TNR', 'FPR', 'FNR', 'P', 'R', 'F1', 'ACC', 'AUROC', 'Sn', 'Sp', 'MCC'], 
                 rule='best', 
                 target_fpr=None,
                 reverse=False) -> None:
        """
    ...
    def _compute_metrics(self, inputs: List[DetectionResult], threshold: float) -> Dict[str, float]:
        ...
        metrics = {
            ...
            # AUROC (only for floats)
            'AUROC': roc_auc_score([int(d.gold_label) for d in inputs], [d.detect_result for d in inputs]) if all(isinstance(d.detect_result, float) for d in inputs) else 0,
            
            'Sn': TP / (TP + FN) if TP + FN else 0,  # Sensitivity
            'Sp': TN / (TN + FP) if TN + FP else 0,   # Specificity
            
            'MCC': (TP * TN - FP * FN) / math.sqrt((TP + FP) * (TP + FN) * (TN + FP) * (TN + FN)) if (TP + FP) * (TP + FN) * (TN + FP) * (TN + FN) else 0
        }
        return metrics
