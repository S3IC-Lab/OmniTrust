...
import mauve # faiss-cpu-1.11.0 and mauve-text-0.4.0
from utils.api.openai_api import OpenAIAPI
from utils.watermark.exceptions.exceptions import CodeExecutionError, InvalidAnswerError
...
class RougeCalculator(ReferencedTextQualityAnalyzer):
    """ROUGE score calculator using rouge-score library.
    
    Supports ROUGE-1, ROUGE-2, ROUGE-L metrics with precision/recall/fmeasure.
    """

    def __init__(self, rouge_types: list = ["rouge1", "rouge2", "rougeL"], use_stemmer: bool = True):
        """
        ...
        Args:
            ...
            use_stemmer: Whether to use Porter stemmer (for English only).
        """
    ...
    def analyze(self, text: str, reference: str, return_all_scores: bool = False):
        """
        Calculate ROUGE scores between generated text and reference text.
        
        Args:
            text: Generated text (usually watermarked)
            reference: Reference/original text
            return_all_scores: If True, returns all sub-scores (precision/recall/fmeasure)
                              If False, returns only F1 scores (default)
        
        Returns:
            dict: ROUGE scores in format {rouge_type: fmeasure} or 
                  {rouge_type: {"precision": p, "recall": r, "fmeasure": f}} if return_all_scores=True
        """
        ...
        if return_all_scores:
            ...
            return {rouge_type: scores[rouge_type].fmeasure for rouge_type in self.rouge_types}
        

class MauveCalculator(ReferencedTextQualityAnalyzer):
    """Mauve score calculator using mauve-text library.
    
    Calculate overall distribution similarity by accumulating generated text and reference text。
    """
    ...
    def __init__(self):
        ...
        self.count = 0  # counters
    
    def analyze(self, text: str, reference: str) -> None:
        """Accumulate generated text and reference text without immediately calculating scores, calculating it when len(*)>=200。
        
        Args:
            text: Generate a single text (with watermarked text)
            reference: Single reference text (original text)
        """
        self.generated_texts.append(text)
        self.reference_texts.append(reference)
        self.count += 1
        # Trigger calculation when reaching 200 pairs
        if self.count >= 200:
            try:
                ...
                score = 0.0
            # Reset counter and accumulate text
            self._reset()
            return score
        else:
            return None
    ...
    def _compute_score(self) -> float:
        """Internal calculation logic ensures paired text matching"""
        min_len = min(len(self.generated_texts), len(self.reference_texts))
        if min_len < 1:
            raise ValueError("Insufficient number of text pairs")
        
        # Call the mauve library to calculate distribution similarity
        result = mauve.compute_mauve(
            p_text=self.generated_texts[:min_len],
            q_text=self.reference_texts[:min_len],
            verbose=False
        )
        return result.mauve
    
    def _reset(self) -> None:
        """Reset accumulated text and counters"""
        self.generated_texts = []
        self.reference_texts = []
        self.count = 0
