...
class OpenAIChat():
    ...
    async def async_run(self, messages_list, expected_type):
        ...
        while retry > 0 and len(messages_list_cur_index) > 0:
            ...
            preds = [self._type_check(self._boolean_fix(prediction.choices[0].message.content.strip()), expected_type) if prediction is not None else None for prediction in predictions]
