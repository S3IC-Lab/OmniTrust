...
from .tool import google_scholar
from modules.hallucination.llm.factool.base import base_pipeline

class scientific_pipeline(base_pipeline):
    ...
    async def _verification(self, claims, responses):
        ...
        for claim, response in zip(claims, responses):
            gen_author = claim.get('paper_author(s)')
            real_author = response.get('author')

            if gen_author is None:
                print("[WARN] claim missing 'paper_author(s)':", claim)
                gen_author = "UNKNOWN"

            authors_pairs.append((gen_author, real_author))

        check_authors_results = await self._check_authors(authors_pairs)

        final_responses = []
        for i, (claim, response) in enumerate(zip(claims, responses)):
            gen_title  = claim.get('paper_title', 'UNKNOWN_TITLE')
            gen_author = claim.get('paper_author(s)', 'UNKNOWN_AUTHOR')
            gen_year   = claim.get('paper_pub_year', 'UNKNOWN_YEAR')

            real_title  = response.get('title', 'UNKNOWN_TITLE')
            real_author = response.get('author', 'UNKNOWN_AUTHOR')
            real_year   = response.get('pub_year', 'UNKNOWN_YEAR')

            final_response = {
                'generated_paper_title': gen_title,
                'generated_paper_author(s)': gen_author,
                'generated_paper_pub_year': gen_year,
                'actual_paper_title': real_title,
                'actual_paper_author(s)': real_author,
                'actual_paper_pub_year': real_year,
            }

            errors = []

            if isinstance(gen_title, str) and isinstance(real_title, str):
                if (
                    gen_title.lower() != real_title.lower()
                    and gen_title.lower() not in real_title.lower()
                    and real_title.lower() not in gen_title.lower()
                ):
                    errors.append('wrong_paper_title')
            else:
                errors.append('wrong_paper_title')

            if not check_authors_results[i].get('factuality', True):
                errors.append('wrong_paper_author(s)')

            if gen_year != real_year:
