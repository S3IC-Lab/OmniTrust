...
# ================================================
# its_edit.py
# Description: Implementation of ITSEdit algorithm
# ================================================
...
from utils.watermark.exceptions.exceptions import AlgorithmNameMismatchError
...
class ITSEditUtils:
    ...
    def transform_key_func(self,generator,n,vocab_size):
        ...
        cpu_generator = torch.Generator(device='cpu').set_state(generator.get_state())
        pi = torch.randperm(vocab_size, generator=cpu_generator)
        xi = torch.rand((n,1), generator=cpu_generator)
        return xi.to(generator.device), pi.to(generator.device)
...
class ITSEdit(BaseWatermark):
    ...
    def __init__(self, algorithm_config: str | ITSEditConfig, transformers_config: TransformersConfig | None = None, *args, **kwargs) -> None:
        if isinstance(algorithm_config, str):
            self.config = ITSEditConfig(algorithm_config, transformers_config)
        elif isinstance(algorithm_config, ITSEditConfig):
            self.config = algorithm_config
        else:
            raise TypeError("algorithm_config must be either a path string or a ITSEditConfig instance")
    ...
    def generate_unwatermarked_text(self, prompt: str, *args, **kwargs) -> str:
        ...
        def generate_rnd(prompts, m, model):
            ...
            for i in range(m):
                ...
                torch.manual_seed(torch.randint(2**32, (1,)).item())
                with torch.no_grad():
                    output = model(inputs)
    ...
    def get_data_for_visualization(self, text: str, *args, **kwargs) -> DataForVisualization:
        """ Get data for visualization of the ITSEdit algorithm. """
        # Logic: When Using this algorithm to generate watermark, the model selects tokens based on the top_k positions in the permutation pi. Therefore, during visualization, if a token is in the top_k positions of pi, it is considered a watermark token and is highlighted accordingly.

        generator = torch.Generator(device='cpu')
        generator.manual_seed(self.config.key)
        _, pi_tensor = self.utils.transform_key_func(generator, n=self.config.pseudo_length, vocab_size=self.config.vocab_size)
        ...
        for tid in token_ids:
            pos = np.where(pi == tid)[0]
            pos = pos[0] if pos.size > 0 else float('inf')
            highlight_values.append(1 if pos < self.config.top_k else 0)
        
        # Decode
        decoded_tokens = [self.config.generation_tokenizer.decode(tid) for tid in token_ids]
