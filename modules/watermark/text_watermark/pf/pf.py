...
class PF(BaseWatermark):
    ...
    def get_data_for_visualization(self, text: str, *args, **kwargs) -> tuple[list[str], list[float]]:
        """Get data for visualization."""
        # Encode
        encoded_text = self.config.generation_tokenizer.encode(text, add_special_tokens=False)
        
        # Get the score of each token
        scores_by_t = self.utils.get_scores_by_t(text, scoring_method="none")
        
        # Initialize the highlighted value list with the same length as the encoded text
        highlight_values = [0.0] * len(encoded_text)
        
        # Update highlighted value list
        start_pos = self.config.ngram + 1
        for idx, scores in enumerate(scores_by_t, start=start_pos):
            # Use the score of the payload index as the highlight value
            highlight_values[idx] = float(scores[self.config.payload])
        
        # Decode
        decoded_tokens = []
        for token_id in encoded_text:
            token = self.config.generation_tokenizer.decode(token_id)
