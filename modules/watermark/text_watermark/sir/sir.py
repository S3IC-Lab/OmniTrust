...
from utils.watermark.utils import create_directory_for_file, load_config_file
...
class SIRUtils:
    ...
    def _get_transform_model(self, model_name: str, input_dim: int) -> TransformModel:
        ...
        model.load_state_dict(torch.load(model_name, weights_only="Ture"))
        return model
...
class SIRLogitsProcessor(LogitsProcessor):
    ...
    def _bias_logits(self, scores: torch.Tensor, batched_bias: list[torch.Tensor]) -> torch.Tensor:
        """Bias the scores for the greenlist tokens."""
        # Ensure all tensors in batched_bias are on the CPU and converted to numpy arrays
        batched_bias_np = [bias.cpu().numpy() for bias in batched_bias]
        batched_bias_np = np.array(batched_bias_np)
        batched_bias_tensor = torch.Tensor(batched_bias_np).to(self.config.device)
        
        # Ensure the size of batched_bias_tensor matches the size of scores
        if batched_bias_tensor.size(1) > scores.size(1):
            batched_bias_tensor = batched_bias_tensor[:, :scores.size(1)]
        elif batched_bias_tensor.size(1) < scores.size(1):
            batched_bias_tensor = torch.nn.functional.pad(batched_bias_tensor, (0, scores.size(1) - batched_bias_tensor.size(1)))
...
class SIR(BaseWatermark):
    ...
    def detect_watermark(self, text: str, return_dict: bool = True, *args, **kwargs):
        ...
        if return_dict:
            return {"is_watermarked": bool(is_watermarked), "score": float(z_score)}
        else:
