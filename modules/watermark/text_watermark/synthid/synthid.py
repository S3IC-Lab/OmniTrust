...
from utils.watermark.utils import load_config_file
from utils.watermark.transformers_config import TransformersConfig
from transformers import LogitsProcessor, LogitsProcessorList
from utils.watermark.visualize.data_for_visualization import DataForVisualization
from utils.watermark.exceptions.exceptions import AlgorithmNameMismatchError, InvalidWatermarkModeError
...
class SynthID(BaseWatermark):
    ...
    def get_data_for_visualization(self, text: str, *args, **kwargs) -> DataForVisualization:
        ...
        # Calculate g value and dynamic threshold
        encoded_text = self.config.generation_tokenizer(text, return_tensors="pt", add_special_tokens=False)["input_ids"].to(self.config.device)
        g_values = self.logits_processor.compute_g_values(encoded_text)
        seq_len = encoded_text.shape[1]
        adjusted_threshold = self.config.threshold * (1 + 0.05 * (seq_len / 100))  # 动态阈值
        
        # Calculate weighted average score
        depth = g_values.shape[2]
        weights = torch.linspace(1.0, 0.5, depth, device=g_values.device)
        ngram_scores = torch.sum(g_values.float() * weights, dim=2) / torch.sum(weights)
        ngram_scores = ngram_scores.squeeze(0).cpu().numpy()
        
        # Generate mask
        eos_mask = self.logits_processor.compute_eos_token_mask(encoded_text, self.config.generation_tokenizer.eos_token_id).squeeze(0).cpu().numpy()
        context_mask = (self.logits_processor.compute_context_repetition_mask(encoded_text).squeeze(0).cpu().numpy()
                        if self.config.watermark_mode == "non-distortionary"
                        else np.ones(ngram_scores.shape[0], dtype=bool))
        
        # Statistically valid n-gram
        highlight_count = np.zeros(seq_len, dtype=int)
        total_count = np.zeros(seq_len, dtype=int)
        
        for ngram_idx in range(ngram_scores.shape[0]):
            start = ngram_idx
            end = ngram_idx + self.config.ngram_len - 1
            if end >= seq_len or not np.all(eos_mask[start:end+1]) or not context_mask[ngram_idx]:
                continue
            total_count[end] += 1
            if ngram_scores[ngram_idx] > adjusted_threshold:
                highlight_count[end] += 1
        
        # Assign highlight value
        highlight_values = []
        for i in range(seq_len):
            if total_count[i] == 0:
                highlight_values.append(0)
            else:
                ratio = highlight_count[i] / total_count[i]
                if ratio > 0.7:
                    highlight_values.append(2)
                elif ratio > 0.3:
                    highlight_values.append(1)
                else:
                    highlight_values.append(0)
        
        # Decode
        decoded_tokens = [self.config.generation_tokenizer.decode([tid]) for tid in encoded_text[0]]
