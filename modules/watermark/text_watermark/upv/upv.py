...
from utils.watermark.exceptions.exceptions import AlgorithmNameMismatchError, InvalidDetectModeError
...
class UPVUtils:
    ...
    def _get_generator_model(self, input_dim: int, window_size: int) -> UPVGenerator:
        ...
        model.load_state_dict(torch.load(self.config.generator_model_name, weights_only="Ture"))
        return model
    ...
    def _get_detector_model(self, bit_number: int) -> UPVDetector:
        ...
        model.load_state_dict(torch.load(self.config.detector_model_name, weights_only="Ture"))
        return model
