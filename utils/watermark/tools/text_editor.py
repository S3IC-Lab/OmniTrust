...
from utils.api.openai_api import OpenAIAPI
from utils.watermark.exceptions.exceptions import DiversityValueError
from utils.watermark.tools.oracle import QualityOracle
...
class GPTParaphraser(TextEditor):
    """Paraphrase a text using the GPT model (compatible with openai>=1.0.0)."""

    def __init__(self, openai_model: str, prompt: str, api_key: str, api_base: str = None) -> None:
        """
            Initialize the GPT paraphraser.

            Parameters:
                openai_model (str): The OpenAI model to use for paraphrasing.
                prompt (str): The prompt to prepend before user text.
                api_key (str): The OpenAI API key for authentication.
                api_base (str, optional): The API base URL for OpenAI (or your custom endpoint).
        """
    ...
    def edit(self, text: str, reference=None):
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are a helpful assistant to rewrite the text."},
...
class TypoAttack(TextEditor):
    """Introduce typos randomly into the text."""

    def __init__(self, typo_rate: float) -> None:
        """
            Initialize the typo attack editor.

            Parameters:
                typo_rate (float): The ratio of characters to alter.
        """
    ...
    def edit(self, text: str, reference=None):
        ...
        typo_count = int(total_chars * self.typo_rate)

        # Introduce typos by randomly altering characters or deleting them
        for _ in range(typo_count):
            idx = random.randint(0, total_chars - 1)
            char = char_list[idx]

            # Randomly decide to either replace or delete the character
            if random.random() < 0.5:
                # Replace the character with a random lowercase letter
                char_list[idx] = random.choice(string.ascii_lowercase)
            else:
                # Delete the character (set to empty string)
                char_list[idx] = ''

        # Rebuild the text with typos
        typo_text = ''.join(char_list)

        return typo_text


class SyntacticReorderAttack:
    ...
    def attack(self, text):
        ...
        for sent in doc.sents:
            transformed = self.active_to_passive(sent)
            if transformed:
    ...
    def active_to_passive(self, sent):
        ...
        if subject and verb and dobj:
            verb_past = self.get_past_participle(verb.lemma_)
            be = "is"  
            passive_sent = f"{dobj.text.capitalize()} {be} {verb_past} by {subject.text}."
            return passive_sent

        return None

    def get_past_participle(self, lemma):
    ...
    def __repr__(self):
        return "SyntacticReorderAttack()"


class ContractionAttack(TextEditor):
    """Contract verbs and auxiliary verbs in the text."""

    def __init__(self, contraction_dict=None) -> None:
        """
        Initialize the contraction attack editor.

        Parameters:
            contraction_dict (dict): A dictionary of common contractions.
        """
        # Default contraction dictionary if none is provided
    ...
    def edit(self, text: str, reference=None):
        ...
        # Try replacing words with their contracted forms
        for long_form, contraction in self.contraction_dict.items():
            text = text.replace(long_form, contraction)
...
class ExpansionAttack(TextEditor):
    ...
    def __init__(self, expansion_dict=None) -> None:
        """
        Initialize the expansion attack editor.

        Parameters:
            expansion_dict (dict): A dictionary of contractions and their expanded forms.
        """
        # Default expansion dictionary if none is provided
    ...
    def edit(self, text: str, reference=None):
        ...
        # Try replacing contractions with their expanded forms
        for contraction, long_form in self.expansion_dict.items():
            text = text.replace(contraction, long_form)

        return text

class MispellingAttack(TextEditor):
    """Introduce common misspellings into the text."""

    COMMON_MISSPELLINGS = {
        "their": "thier", "receive": "recieve", "separate": "seperate",
        "definitely": "definately", "occurred": "occured", "accommodate": "accomodate",
        "government": "goverment", "independent": "independant", "existence": "existance",
        "embarrass": "embarass", "privilege": "priviledge", "questionnaire": "questionaire",
        "successful": "succesful", "conscience": "concience", "recommend": "reccomend",
        "acknowledge": "aknowledge", "argument": "arguement", "category": "catagory",
        "foreign": "foriegn", "harass": "harrass", "mischievous": "mischevious",
        "noticeable": "noticable", "playwright": "playwrite", "possession": "posession",
        "relevant": "relevent", "supersede": "supercede", "threshold": "threshhold",
        "assassin": "asassin", "balloon": "baloon", "believe": "beleive",
        "brilliant": "briliant", "business": "buisness", "calendar": "calender",
        "caribbean": "carribean", "cemetery": "cemetary", "chauffeur": "chauffer",
        "colleague": "collegue", "committee": "commitee", "completely": "completly",
        "convenience": "convinience", "curiosity": "curiousity", "decision": "decicion",
        "discipline": "disipline", "disappear": "dissapear", "disappoint": "dissapoint",
        "ecstasy": "ecstacy", "equipment": "equiptment", "exaggerate": "exagerate",
        "familiar": "familar", "fascinating": "facinating", "grateful": "greatful",
        "guarantee": "garantee", "hygiene": "hygine", "humorous": "humourous",
        "innocent": "innosent", "intelligence": "inteligence", "jewelry": "jewelery",
        "knowledge": "knowlege", "lightning": "lightening", "maintenance": "maintainance",
        "maneuver": "manuever", "millennium": "milennium", "misspell": "mispell",
        "negligible": "neglible", "occasion": "ocassion", "parallel": "paralel",
        "pastime": "pasttime", "perceive": "percieve", "permanent": "perminant",
        "personnel": "personel", "playwright": "playwrite", "prejudice": "predjudice",
        "professor": "proffesor", "questionnaire": "questionaire", "repetition": "repetion",
        "restaurant": "restaraunt", "rhythm": "rythm", "schedule": "schedual",
        "sense": "sence", "siege": "seige", "speech": "speach",
        "strengthen": "strenghten", "surprise": "suprise", "technical": "tecnical",
        "temperature": "temprature", "twelfth": "twelth", "vacuum": "vaccum",
        "weather": "wheather", "weird": "wierd", "whisper": "whispher"
    }

    def __init__(self, error_rate: float) -> None:
        """
        Initialize the misspelling attack editor.

        Parameters:
            error_rate (float): The probability of replacing a word with its misspelled version.
        """
        self.error_rate = error_rate

    def edit(self, text: str, reference=None):
        """
        Introduce misspellings into the text.

        Parameters:
            text (str): The input text to be modified.
            reference: Unused parameter, included for compatibility.

        Returns:
            str: The modified text with some words replaced by their misspelled versions.
        """

        if not text:
            return text

        # Extract words from the text using regex (ignoring punctuation)
        ...
        for word in words:
            ...
            # Replace the word with its misspelled version if it exists in the dictionary and meets the error rate threshold
            if lower_word in self.COMMON_MISSPELLINGS and random.random() < self.error_rate:
                misspelled = self.COMMON_MISSPELLINGS[lower_word]
                # Preserve capitalization if the original word is capitalized
                altered_words.append(misspelled if word.islower() else misspelled.capitalize())
            else:
                altered_words.append(word)

        # Reconstruct the text with the altered words while maintaining original formatting
        return re.sub(r'\b\w+\b', lambda m: altered_words.pop(0), text)

class ModifyAttack:
    """Randomly modify words in the text by duplication, removal, or synonym replacement."""

    def __init__(self, modification_prob=0.1) -> None:
        """
        Initialize the modify attack editor.

        Parameters:
            modification_prob (float): Probability for each modification (duplicate, remove, replace).
        """
    ...
    def edit(self, text: str, reference=None):
        """Modify words in the text by duplication, removal, or synonym replacement."""
        ...
        for word in words:
            if random.random() < self.modification_prob:
                # Randomly choose a modification type (duplicate, remove, replace)
                modification_type = random.choice(["duplicate", "remove", "replace"])

                if modification_type == "duplicate":
                    # Duplicate the word
                    modified_words.append(word)
                    modified_words.append(word)
                elif modification_type == "remove":
                    # Skip adding the word (remove it)
                    continue
