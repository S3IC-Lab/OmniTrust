...
from .color_scheme import ColorScheme, ColorSchemeForDiscreteVisualization, ColorSchemeForContinuousVisualization, ColorSchemeForMultiLevelVisualization
...
class MultiLevelDiscreteVisualizer(BaseVisualizer):
    """Visualizer for multi-level discrete visualization (SynthID)."""
    def __init__(self, 
                 color_scheme: ColorSchemeForMultiLevelVisualization,
                 font_settings: FontSettings,
                 page_layout_settings: PageLayoutSettings,
                 legend_settings: DiscreteLegendSettings) -> None:
    ...
    def _highlight_single_token(self, draw, token, value, token_width, show_text, x, y):
        """Highlight a token based on its value."""
        color_mapping = {0: self.color_scheme.non_watermark_color,
                         1: self.color_scheme.weak_watermark_color,
                         2: self.color_scheme.strong_watermark_color}

        color = color_mapping.get(value, self.color_scheme.prefix_color)
    ...
    def _display_legend(self, draw, x, y, img_height=None):
        """Display the legend for multi-level discrete visualization."""
        items = self.color_scheme.get_legend_items()
        rect_width = self.legend_settings.rec_width
        rect_height = self.font_settings.font_size
        text_offset = self.legend_settings.text_offset
        ...
        for label, color in items:
            ...
            draw.text((x + rect_width + text_offset, y + rect_height/2 - self.legend_settings.legend_font_size/2),
                label, 
                fill="black", 
                font=self.legend_settings.legend_font)
            y += rect_height + text_offset
